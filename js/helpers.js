/* firebase helper functions for mm350*/
...
fb.create = function (userName, email, password) {
    firebase.auth().createUserWithEmailAndPassword(email, password).then(credential =>{
        const ref = firebase.database().ref('users').child(credential.user.uid).update({displayName: userName}).then(()=> {location.href = "index.html"});
    })
    .catch(error => {fb.onError(error.message);});
};
/*logs into firebase, calls onError if there's an error */
fb.login = function(email, password){
    firebase.auth ().signInWithEmailandPassword(email, password).then(()=> {console.log('success');}).catch(error)=>{fb.onError(error.message);});
};
